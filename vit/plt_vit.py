...
labels = [r"IVON $h_0=0.2$", r"IVON $h_0=0.5$", r"IVON $h_0=0.8$"]
...
# Initialize empty lists to load data one at a time
train_accuracies = []
test_accuracies = []
eigenvalues = []
...
for i, train_acc in enumerate(train_accuracies):
    axs[0].plot(
        ...
        marker=markers[i], color=colors[i], linestyle=linestyles[i], linewidth=2, markersize=6
    )
...
axs[1].set_ylim(0.54, 0.63)  # Restrict y-axis for Test Accuracy

# Set less dense grids by customizing ticks
x_ticks = np.arange(0, 110000, 20000)  # X-axis ticks every 20,000
y_ticks = np.arange(0.54, 0.64, 0.02)  # Y-axis ticks every 0.02
axs[1].set_xticks(x_ticks)
axs[1].set_yticks(y_ticks)

axs[1].tick_params(axis='both', which='major', labelsize=tick_fontsize)
axs[1].grid(True, which='major', linewidth=1)  # Major gridlines only
...
for i, eigs in enumerate(eigenvalues):
    axs[2].scatter(
        ...
        color=colors[i], marker=markers[i], s=40, alpha=0.8
    )
# Add dashed line at 400
